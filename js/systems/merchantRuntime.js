...
export function createWorkersRuntime({
  ...
  const farmSystem = createFarmSystem({
    ...
    spendResource,
    spendCosts,
    getManagementMaterialDiscount: () => getManagementMaterialDiscount()
  });
  ...
  function getWorkerCycleTime(job = "idle") {
    ...
    if (job === "farming") return 10;
    if (job === "crafting") return 10;
    if (job === "cook") return 10;
    if (job === "ranch") return 10;
    return 10;
  }
  ...
  function workerEatIfNeeded(worker, needed) {
    ...
    const actual = Math.min(Math.max(0, worker.maxStamina - worker.stamina), recover, Math.max(recover, missing));
