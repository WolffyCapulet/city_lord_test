...
export function renderActionLane({
  ...
  const textEl = document.getElementById("productionText");
  const barEl = document.getElementById("productionBar");
  ...
  if (state.currentAction && workDefs[state.currentAction.id]) {
    ...
    textEl.textContent = `生產中：${def.name}｜剩餘 ${formatSeconds(remaining)}`;
    ...
    textEl.textContent = "生產線：目前沒有進行中的動作。";
    barEl.style.width = "0%";
  }
        ...
        .map((item, index, arr) => {
          ...
          return `
            <div class="queue-row">
              ...
              <div class="ops">
                <button type="button" class="tiny-btn" data-action-up="${index}" ${index === 0 ? "disabled" : ""}>↑</button>
                <button type="button" class="tiny-btn" data-action-down="${index}" ${index === arr.length - 1 ? "disabled" : ""}>↓</button>
                <button type="button" class="tiny-btn" data-action-remove="${index}">×</button>
              </div>
            </div>
          `;
        })
